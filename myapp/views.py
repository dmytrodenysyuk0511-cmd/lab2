...
def home(request):
    ...
    categories = Category.objects.prefetch_related('posts').annotate(
        topics_total=Count("posts", distinct=True),
        messages_total=Count("posts__comments", distinct=True),
    ).order_by("name")

    posts = (
        Post.objects.select_related("category", "author", "author__forum_profile")
...
def category_posts(request, category_id):
    ...
    category = get_object_or_404(Category.objects.prefetch_related('posts'), id=category_id)
...
def post_detail(request, post_id):
    ...
    messages = list(
        ...
        .select_related("author", "author__forum_profile", "parent", "parent__author", "parent__author__forum_profile")
        .order_by("created_at")
    )
    ...
    for message in messages:
        ...
        if message.parent and message.parent.author_id:
            author_ids.add(message.parent.author_id)

    profiles_by_user_id = {
        profile.user_id: profile
        for profile in UserProfile.objects.filter(user_id__in=author_ids).select_related('user')
    }
...
def account_settings_view(request):
    ...
    if request.method == "POST":
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest' or request.content_type == 'application/x-www-form-urlencoded':
            ...
            return JsonResponse({"status": "success", "theme": theme, "language": language})
...
def store_home(request):
    ...
    product_categories = ProductCategory.objects.prefetch_related('products').annotate(
        products_total=Count("products", distinct=True)
    ).order_by("name")

    products = Product.objects.select_related("category").prefetch_related('ratings').order_by("name")
...
def product_detail(request, product_id):
    ...
    ratings = (
        ...
        .select_related("user", "user__forum_profile")
        .order_by("-updated_at")
    )
...
def friends_list(request):
    ...
    # Отримати всіх друзів (прийняті запити)
    ...
    ).distinct().select_related('forum_profile')

    # Вхідні запити
    incoming_requests = Friendship.objects.filter(
        to_user=request.user,
        status='pending'
    ).select_related('from_user', 'from_user__forum_profile')

    # Вихідні запити
    outgoing_requests = Friendship.objects.filter(
        from_user=request.user,
        status='pending'
    ).select_related('to_user', 'to_user__forum_profile')
    ...
    if search_query:
        # Знайти користувачів за ніком (виключаючи себе)
        search_results = User.objects.filter(
            username__icontains=search_query
        ).exclude(
            id=request.user.id
        ).select_related('forum_profile')[:10]

        # Додати інформацію про статус дружби для кожного користувача
        friends_ids = friends.values_list('id', flat=True)
        outgoing_ids = outgoing_requests.values_list('to_user_id', flat=True)
        incoming_ids = incoming_requests.values_list('from_user_id', flat=True)
...
def chat_with_friend(request, username):
    if not request.user.is_authenticated:
        return redirect("login_view")
    ...
    # Отримати всі повідомлення між користувачами
    messages = DirectMessage.objects.filter(
        Q(sender=request.user, receiver=friend) |
        Q(sender=friend, receiver=request.user)
    ).select_related('sender', 'sender__forum_profile').order_by('created_at')
    ...
    if request.method == 'POST':
        ...
        if text:
            ...
            return redirect('chat_with_friend', username=username)

    context.update({
        'title': f'Чат з {friend.username}',
        'friend': friend,
        'messages': messages,
    })
